...
# NOTE (ops#165): the src/lib extensionless-import check lived here until the
# read-only rewrite deleted src/ and scripts/test_web.mts. The site is now
# dependency-free static files (index.html, styles.css, app.js); nothing is
# loaded by Node's ESM loader anymore, so the check has no subject.
